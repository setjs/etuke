            ...
            <form id="form">
                <div class="form-horizontal form-item">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            @include('components.backend.datetime', ['name' => 'published_at' , 'value'=>"$one->published_at"])
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-5" id="select-tag" style="padding-top: 4px">
                            <input type="hidden" name="tag_id" value="{{$one->tag_id}}" />
                            @foreach($tag as $val)
                                @if(in_array($val->id , $has))
                                    <span class="tag active" data-id="{{$val->id}}">{{$val->name}}</span>
                                @else
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <input type="text" name="amount" value="{{$one->amount}}" class="form-control" placeholder="价格" required>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <select multiple style="width: 300px" name="platform">
                                @foreach($platform as $val)
                                    @if($one->platform == $val->name)
                                    <option value="{{$val->name}}" selected="selected">{{$val->name}}</option>
                                    @else
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <textarea name="seo_keywords" class="form-control" rows="2" placeholder="SEO关键字" required>{{$one->seo_keywords}}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label"><span style="color: red">*</span> SEO描述:  </label>
                        <div class="col-sm-5">
                            <textarea name="seo_description" class="form-control" rows="2" placeholder="SEO描述" required>{{$one->seo_description}}</textarea>
                        </div>
                    </div>
                    @include('components.backend.image', ['name' => 'thumb', 'title' => '封面','value'=> "$one->thumb"])
...
@section('script')
    ...
    <script>
        ...
        $('#submit-form').click(function(){
            ...
            APP.ajax({'url':"{{ route('post.photo.update')}}"},'#form',function(r){
                if(r.code == 500){
                    console.log(r);
                  // alert('请填检查相应的数据');
