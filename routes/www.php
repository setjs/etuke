...
Route::any('/register.html', 'Auth\RegisterController@showRegistrationForm')->name('register');
...
Route::any('/login.html', 'Auth\LoginController@login')->name('login');



Route::group(['namespace' =>'Pc','middleware' =>'user.info' ],function ($router){

    Route::get('/', 'IndexController@index')->name('index');
    Route::get('detail/{id}.html', 'IndexController@detail')->name('detail.index');
    Route::get('detail/ajax/{id}.html', 'IndexController@detail')->name('detail.ajax');
    Route::get('tags/{id}.html', 'TagsController@index')->name('get.tags.index');
...
//Route::post('/subscription/email', 'Frontend\IndexController@subscriptionHandler')->name('subscription.email');
    ...
    Route::get('/faq/category/{id}', 'FaqController@showCategoryPage')->name('faq.category.show');
    Route::get('/faq/article/{id}', 'FaqController@showArticlePage')->name('faq.article.show');

    Route::get('/books.html', 'BookController@index')->name('books');
    Route::get('/book/{id}.html', 'BookController@show')->name('book.show');

// 支付回调
    Route::post('/payment/callback/{payment}', 'Frontend\PaymentController@callback')->name('payment.callback');


    Route::get('double/map.html', 'DoubleBallController@doubleMap')->name('double.map');

    Route::get('double', 'DoubleBallController@index')->name('double.index');
    Route::get('double/list.html', 'DoubleBallController@list')->name('double.list');
    Route::get('double/{week}.html', 'DoubleBallController@week')->name('double.list');

    // http://pecl.php.net/get/imagick-3.4.4.tgz   /etc/init.d/php-fpm restart  find / -name fileinfo

     // ln -s /opt/itoutu.cn/storage/app      /opt/itoutu.cn/public/uploads
...
//Route::group(['prefix' => 'my' ,'namespace' =>'Web','middleware' =>['user.info' ]],function ($router){
//
//});
...
Route::group(['prefix' => '/open', 'namespace' => 'Open'], function () {
    ...
    Route::get('', 'IndexController@index')->name('open.index');
